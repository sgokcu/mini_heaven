...
void take_name(char *args, t_mini *mini)
{
    ...
    while(args[i] && (quote_check(args[i], &mini->utils->squote, &mini->utils->dquote), 1))
    {
        if ((mini->utils->dquote % 2 == 0 && mini->utils->squote % 2 == 0)
            && ((args[i] == ' ' || args[i] == '\t')
            || (args[i] == '<' || args[i] == '>')))
            break ;
        i++;
...
char *delete_quotes(char *str, t_mini *mini)
{
	...
	while(str[i] && (quote_check(str[i], &sq, &dq), 1))
		i++;
	j = ft_strlen(str) - (mini->utils->squote + mini->utils->dquote);
	...
	while(str[i] && (quote_check(str[i], &sq, &dq), 1))
	{
		if((str[i] == 39 && dq % 2 == 0) || (str[i] == 34 && sq % 2 == 0))
			i++;
		else
			hold[j++] = str[i++];
	}
